...
```bash
go get github.com/pintomau/ringring
```
...
rb, _ := ringring.NewRingBuffer[int](ctx, 1024) // capacity rounds up to a power of 2
...
slotID, _ := s.AddReader(func(ctx context.Context, rv ringring.ReadView[int], cur *atomic.Int64) {
	expected := cur.Load() + 1
	for {
		select {
		case <-ctx.Done():
...
- **Idle with `time.Sleep`, not `runtime.Gosched()`.** A `Gosched` idle loop keeps the goroutine in the
  run queue and starves the writer; a 50µs sleep parks it properly. The reasoning and measurements are
...
```go
rb, _ := ringring.NewRingBuffer[Event](ctx, 1<<16,
	ringring.WithWaitStrategy[Event](ringring.WaitStrategySpin))
```
...
|----------------|-------------|-----|
| Writer backpressure (`WithWaitStrategy`) | `Yield` (default); `Spin` when latency-critical | the buffer-full path is rare when the ring is sized right, so politeness is cheap; spin only buys the last microseconds |
...
The two findings that look contradictory at first glance both follow from this: **sleeping is the
right way to idle** (it parks the goroutine, protecting throughput) **and the wrong way to poll**
