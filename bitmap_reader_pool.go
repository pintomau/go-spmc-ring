...
type BitmapReaderPool[T any] struct {
	...
	// # Cache line 1: Allocation Metadata + Backoff Config
	allocatedSlots [2]atomic.Uint64 // 16 bytes - slot allocation bitmap
	// Backoff config
	initialBackoff     time.Duration //  8 bytes - initial idle backoff duration
	maxBackoff         time.Duration //  8 bytes - maximum backoff cap
	backoffMultiplier  float64       //  8 bytes - exponential backoff multiplier
	terminateThreshold int           //  8 bytes - idle loops before goroutine self-terminates
	_                  [CacheLineSize - 16 - 8 - 8 - 8 - 8]byte
	...
	buffer       []T                // 24 bytes - backing array (set once at construction)
	writerCursor WriterBarrier      // 16 bytes - upstream barrier
	mask         int64              //  8 bytes - ring mask (set once at construction)
	rootCancel   context.CancelFunc //  8 bytes - Global shutdown trigger
	// # Another cache line on x86-64
	rootCtx context.Context // 16 bytes - Root context for all slots
	wg      sync.WaitGroup  // 16 bytes - Wait for internal goroutines
}
...
func NewBitmapReaderPool[T any](ctx context.Context, writerCursor WriterBarrier) *BitmapReaderPool[T] {
	...
	// Initialize backoff config with defaults
	b.initialBackoff = 10 * time.Millisecond
	b.maxBackoff = 100 * time.Millisecond
	b.backoffMultiplier = 2.0
	b.terminateThreshold = int(1 * time.Minute / (10 * time.Millisecond))
...
func (b *BitmapReaderPool[T]) runSlot(slotId int) {
	var (
		control            = &b.slotControl[slotId]
		terminateThreshold = b.terminateThreshold
		backoffMultiplier  = b.backoffMultiplier
		currentBackoff     = b.initialBackoff
		idleLoops          = 0
	)
	...
	for {
		...
		if currentState&slotStateActive == 0 {
			...
			if currentBackoff > b.maxBackoff {
				currentBackoff = b.maxBackoff
			}
