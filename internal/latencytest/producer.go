...
func (p FixedRateProducer) Run(ctx context.Context, rb *spmc.RingBuffer[Payload]) {
...
func (p BurstProducer) Run(ctx context.Context, rb *spmc.RingBuffer[Payload]) {
...
func (p BurstReserveProducer) Run(ctx context.Context, rb *spmc.RingBuffer[Payload]) {
	idle := time.Duration(p.IdleMs) * time.Millisecond
	n := int64(p.BurstSize)
	var seq int64
	for {
