...
func makeConsumerFn(rec *LatencyRecorder, polling ReaderPolling, sleepPerEvent time.Duration) spmc.ReaderFunc[Payload] {
	if polling == BatchReader {
		return makeBatchConsumerFn(rec)
	}
	return func(ctx context.Context, rv spmc.ReadView[Payload], cur *atomic.Int64) {
		expected := cur.Load() + 1
		for {
			select {
			case <-ctx.Done():
...
// visible.
func makeBatchConsumerFn(rec *LatencyRecorder) spmc.ReaderFunc[Payload] {
	return func(ctx context.Context, rv spmc.ReadView[Payload], cur *atomic.Int64) {
		expected := cur.Load() + 1
		for {
			select {
