...
import (
	...
	"github.com/pintomau/go-spmc-ring/internal/latencytest"
)
...
func parseWait(s string) spmc.WaitStrategy {
	switch s {
	case "spin":
		return spmc.WaitStrategySpin
	case "sleep":
		return spmc.WaitStrategySleep
	case "hybrid":
		return spmc.WaitStrategyHybrid
	default:
		return spmc.WaitStrategyYield
	}
}
...
func collectMatrix(duration time.Duration) []MatrixRow {
	...
	waits := []spmc.WaitStrategy{
		spmc.WaitStrategySpin,
		spmc.WaitStrategyYield,
		spmc.WaitStrategySleep,
		spmc.WaitStrategyHybrid,
	}
