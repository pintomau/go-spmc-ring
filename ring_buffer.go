...
func WithDefaultWaitStrategy[T any]() RingBufferOption[T] {
	return func(r *RingBuffer[T]) {
		r.writerWaitStrategy = WaitStrategyYield
	}
}

func WithDefaultWaitStrategyParams[T any]() RingBufferOption[T] {
	return func(r *RingBuffer[T]) {
		r.writerWaitStrategyParams.SleepDuration = time.Microsecond
		r.writerWaitStrategyParams.HybridSpinCount = 100
		r.writerWaitStrategyParams.HybridSleepDuration = time.Microsecond
	}
}

func WithBackoffParams[T any](params BackoffParams) RingBufferOption[T] {
	return func(r *RingBuffer[T]) {
		r.barrier.initialBackoff = params.InitialBackoff
		r.barrier.maxBackoff = params.MaxBackoff
		r.barrier.backoffMultiplier = params.BackoffMultiplier
		r.barrier.terminateThreshold = int(params.TerminateAfter / params.InitialBackoff)
	}
}

func WithDefaultBackoffParams[T any]() RingBufferOption[T] {
	return func(r *RingBuffer[T]) {
		r.barrier.initialBackoff = 10 * time.Millisecond
		r.barrier.maxBackoff = 100 * time.Millisecond
		r.barrier.backoffMultiplier = 2.0
		r.barrier.terminateThreshold = int(time.Minute / r.barrier.initialBackoff)
	}
}
...
func NewRingBuffer[T any](ctx context.Context, capacity int64, opts ...RingBufferOption[T]) (*RingBuffer[T], error) {
	...
	WithDefaultWaitStrategy[T]()(r)
	WithDefaultWaitStrategyParams[T]()(r)
	WithDefaultBackoffParams[T]()(r)
...
func (r *RingBuffer[T]) Reserve(n int64) (seg1, seg2 []T, claim int64) {
	if n <= 0 || n >= r.bufferSize {
		panic("spmc: Reserve n out of range (must satisfy 0 < n < bufferSize)")
	}
...
func (r *RingBuffer[T]) TryReserve(n int64) (seg1, seg2 []T, claim int64, ok bool) {
	if n <= 0 || n >= r.bufferSize {
		panic("spmc: TryReserve n out of range (must satisfy 0 < n < bufferSize)")
	}
	lastSeq := r.nextSequence + n
	if lastSeq >= r.cachedSlowestReader+r.bufferSize {
		// one refresh, then give up instead of waiting
